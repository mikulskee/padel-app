...
import { filterMatchesByYear, getCurrentYear } from "@/lib/seasons";
...
export default async function PlayerPage({
	...
	const seasonYear = season && /^\d{4}$/.test(season) ? Number(season) : currentYear;
	const isCurrentSeason = seasonYear === currentYear;

	const allMatches = await fetchMatches();
	const seasonMatches = filterMatchesByYear(allMatches, seasonYear);
	...
	const playerStats = calculatePlayerStats(seasonMatches);

	const placeInRanking =
		playerStats.findIndex((player) => player.name.toLowerCase() === playerName.toLowerCase()) +
		1;
	...
	const isPlayerInMatch = (match: Match) =>
		match.players["1"].includes(playerName) || match.players["2"].includes(playerName);

	const sumPoints = (matchesToSum: Match[]) =>
		matchesToSum.filter(isPlayerInMatch).reduce((sum, match) => {
			const { points } = getPlayerPoints(playerName, match);
			return sum + (points || 0);
		}, 0);

	const seasonPoints = sumPoints(seasonMatches);
	const allTimePoints = sumPoints(allMatches);

	const playerSeasonMatches = [...seasonMatches]
		.sort((a, b) => a.date.localeCompare(b.date))
		.reverse()
		.filter(isPlayerInMatch);
	...
	return (
		...
		>
			...
			>
				<Link
					href={isCurrentSeason ? "/" : `/${seasonYear}`}
					style={{
						textDecoration: "underline",
						color: "#a4a4a4",
					}}
				>
					{isCurrentSeason ? "Główna" : `Sezon ${seasonYear}`}
				</Link>{" "}
				&gt;
				<span
					style={{
			...
			>
				...
				<div>
					...
					<p style={{ fontSize: "1rem" }}>Sezon: {seasonYear}</p>
			...
			<table cellPadding={5} style={{ fontSize: "0.8rem", width: "100%" }}>
				<thead></thead>
				<tbody>
					{playerSeasonMatches.map((match, i) => {
						const { points, isInTeam1, isInTeam2, hasPlayerWon } = getPlayerPoints(
							playerName,
							match,
						);
						...
						return (
							<Fragment key={match.id}>
								...
								{i < playerSeasonMatches.length - 1 && (
									<tr>
										<td
											colSpan={5}
											style={{
					...
					<tr>
						<td colSpan={5} style={{ textAlign: "right", fontWeight: "bold" }}>
							Suma punktów (sezon {seasonYear}):
						</td>
						<td style={{ textAlign: "center", width: "10%" }}>{seasonPoints} pkt</td>
					</tr>
					<tr>
						<td colSpan={5} style={{ textAlign: "right", fontWeight: "bold" }}>
							Suma punktów (wszystkie sezony):
						</td>
						<td style={{ textAlign: "center", width: "10%" }}>{allTimePoints} pkt</td>
					</tr>
				</tbody>
			</table>
		</main>
