...
import { filterMatchesByYear } from "@/lib/seasons";
...
export const calculatePlayerStats = (matches: Match[]): PlayerStats[] => {
	...
	const awardPoints = (players: string[], points: number, setsWon: number, setsLost: number) => {
...
export async function UsersTable({
	...
	const previousStats = previousMatches
		? calculatePlayerStats(filterMatchesByYear(previousMatches.matches, activeYear))
		: [];
	...
	const currentStatsWithTrend: PlayerStatsWithTrend[] = playerStats.map((player, index) => {
	...
	return (
		<>
								...
								<td>
									...
									>
										...
										<Link
											...
											href={`/players/${playerId}?season=${activeYear}`}
										>
											{player.name}
										</Link>
										{player.trend === "up" && (
			...
			>
				Punktacja: 2:0 - 25.00 pkt | 2:1 - 20.00 pkt | 1:2 - 10.00 pkt | 0:2 - 5.00 pkt.
			</p>
		</>
	);
}
